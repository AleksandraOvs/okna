;(function($) {
	...
	PPLoginForm = function( $scope, elementSettings, settings ) {
		...
		this.form            = $( this.node ).find( '.pp-login-form' );
		this.passwordField   = this.form.find( '.pp-lf-field-pw-toggle' );
		this._init();
	};
	...
	PPLoginForm.prototype = {
		...
		_init: function() {
			if ( this.passwordField.find( '.pp-lf-toggle-pw' ).length > 0 ) {
				this.passwordField.find( '.pp-lf-toggle-pw' )
		...
		_passwordToggle: function() {
			var pwField = this.passwordField;
	...
	PPGoogleLoginEndpoint = function( googleUser , clientId ) {
		console.log(PPLoginForm.gtheme);
	}
